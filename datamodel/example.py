...
def collate_examples(
    ...
    res = {
        'input_ids': pad_sequence(all_input_ids, batch_first=True, padding_value=padding_token_id).long(),
        'labels': pad_images(target_label_ids, padding_value=-100, padding_length=(pad_length, None)) if not no_target_label_ids else None
    }
...
def strided_split(
    ...
    if entities is not None:
        ...
        target_label_ids = torch.full((text_length, max_entity_length), fill_value=no_entity_category_id, dtype=torch.long).long()
